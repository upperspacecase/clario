...
A voice-agent-driven AI opportunity assessment product. Prospects visit gethours.org, have a 12-minute conversation with an AI agent (Annie), and receive a personalized report at a private share link identifying tools that will save them time, plus upsell opportunities for deeper engagements.
...
3. Browser opens a voice session with Annie (Gemini Flash Live)
...
| `/start` | Pre-call form + Annie voice widget | Public |
...
| `/admin/prompts` | Annie system prompt management (versioned) | Auth-gated |
...
- **Session length**: Live API caps audio sessions at 15 min and underlying connections at ~10 min. Strategy: target a 12-minute conversation. Implement session resumption (handle-based) for safety, but design Annie's prompt to wrap before the limit is reached.
...
- **Identity** — "You are Annie, an AI assistant for GetHours. You help business owners identify where AI can give them their time back."
...
Flash Live supports function calling. Tempting to have Annie call `log_pain_point()` live during the conversation for cleaner structured data. Defer this to v2. v1 captures raw transcript only and extracts in the skills layer — keeps the voice and intelligence layers cleanly separated for debugging.
...
/prompts/{promptId}                          — versioned Annie system prompts
...
```typescript
{
  ...
  // Tracking
  promptVersionId: string;             // which Annie prompt was used
...
- After Annie's wrap or user-clicked end → POST `/api/voice/finalize` → swap to "Thanks, your report is on the way" screen with expected delivery time
...
For in-progress calls. Streams `assessments/{id}/transcript/` in real-time. Lets Tay watch Annie work. Crucial in the first 20–50 calls for prompt iteration.
...
List of all Annie prompt versions. Active one is highlighted. Editor to create new version. "Activate" button. Per-version metrics (call count, avg pain points extracted, avg duration).
...
- [ ] `/admin/prompts` — Annie prompt versioning
